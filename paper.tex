...
\e{\delta S = 2\pi k_E \f{2\alpha m}{\hbar} \ll  S =\pi k_E^2; \;\; k_{E}:=\f{\sqrt{2m E}}{\hbar} .\la{deltaSvsS}}
$S$ is equivalently the area of the circular, constant-energy contour in the absence of spin-orbit coupling ($\alpha{=}0$).

Throughout this work we will treat both $\delta S/S$ and $1/l^2S$  as small parameters with finite ratio $l^2\delta S$ \textbf{[define magnetic length]}. Our perspective diverges from and generalizes previous semiclassical theories [Kohn, Blount, Roth, Wannier, Fredkin, Niu, Mikitik, topoferm, 100page] having only a single small parameter ($B$).  
...
Let us apply a magnetic field, and first consider the spin-degenerate Landau levels obtained by neglecting both $\delta H$ and the Zeeman splitting. In the  semiclassical description,  we are interested in  field-induced dynamics within a low-energy degenerate band of $\hat{H}_0$ with  dispersion $\var(\bk)$, e.g., $\var(\bk){=}\hbar^2 \bk^2/2m{+}O(k^4)$ in the Rashba model.    Landau quantization is well-known to derive from the Peierls substitution $\var(\bk){\rightarrow}\var(\bK)$,[Peierls] with   the kinematic quasimomentum operator   $\boldsymbol{K}{:}{=}\boldsymbol{k}{+}(e/\hbar) \boldsymbol{A}(i\nabla_{\boldsymbol{k}})$ and $\boldsymbol{A}(\br)$ being the electromagnetic vector potential. Assuming that the field is aligned in the ${-}z$ direction, $[K_x,K_y]{=}i\lmt$ with the magnetic length $l{:}{=}\sqrt{\hbar/{eB}}$. The semiclassical solution corresponds to  cyclotron revolutions of an electron around a constant-energy contour of $\var(\bk)$ [see Fig. \ref{fig:orbits}(b)]]; this contour, equipped with an orientation given by the equation of motion ($ \hbar dk_{\alpha}/dt {=} \lmt \epsilon_{\ab}\partial \var/\partial k_{\beta} $, with $\epsilon_{xy}{=}{-}\epsilon_{yx}{=}1$), shall be referred as the zeroth-order orbit $\frako_0$.  In this work, we focus only on closed orbits which do not wrap 
...
To the first order, $\delta H$ and the Zeeman interaction  induce a subleading phase offset ($\lambda_a$) which generically differs for the two orthogonal spinor states (labelled $a{=}1,2$):
\begin{equation}
l^2S(E)+\lambda_a(E,B)+\gamma=2\pi n,\as  n\in \mathbb{Z},\;a=1,2. \label{eq:rule}
\end{equation}
This implies that the spin-degeneracy of Landau levels is split   by  $\var_c|\lambda_1{-}\lambda_2|/2\pi$. The phase offsets $\lambda_a$ can be viewed as Floquet quasienergies of a time-periodic, perturbative Hamiltonian ($\calh$) that governs the dynamics of the spinor wavefunction.  $\lambda_a$ is obtained  from  eigenvalues $\{e^{i\lambda_a}\}_{a=1}^2$ of a time-evolution propagator -- the time-ordered exponential 
\begin{equation}
...
\caption{(a) \textbf{[some axes will be helpful]} Solid (resp.\ dashed) lines respectively illustrate the band dispersion of an electron gas with (resp.\ without) Rashba spin-orbit coupling.  The oriented circles illustrate the spin-orit-split orbits for a magnetic field pointing to $-z$ direction. \textbf{[might replace Delta with $2\alpha k_E$ at some point]} (b) The zeroth-order orbit $\frako_0$ is indicated by a dashed circle with area $S$; $\delta S$ is the differential area  between two spin-orbit-split orbits. The spin texture is indicated by purple arrows.\label{fig:orbits}}
\end{figure}
...
\e{\lambda_{\sma{\pm}} = \int_0^{T_c}\calh_{\sma{\pm\pm}}dt=\pm \tf1{2}l^2\delta S +B\int_0^{T_c}\calm_{\sma{\pm\pm}}dt.  \la{phaseindependentorbit}} 
...
\e{\calh(\bk)=\alpha k\tau_z{+}\f{g_s}{2}\mu_{\sma{B}}B\tau_x \red{-} \f{eB\red{\hbar}}{2m} (\tau_x-I) \epsilon_{\alpha\beta} k_{\red{\alpha}}\nabla_{\red{\beta}}\theta,\la{rashbaeffham}}
...
Due to the continuous rotational symmetry of $H_R$, $\nabk \theta$ has magnitude $1/k$ and lies tangential to $\frako_0$; consequently, $\calh$ is constant along $\frako_0$, and the propagator simplifies to $\cala{=}e^{{-}i\calh T_c}$ without time-ordering, and the phase corrections to the quantization rule are simply the eigenvalues of $\calh$ multiplied by $T_c$:
\begin{equation}
...
In the weak-hybridization regime: $l^2\delta S {>}\pi(g_{\sma{\text{eff}}}{-}2)$, we may expand \q{eq:Rashba-lambda} as a Laurent series in $B$:
\e{\lambda_{\pm}= \pm \f{l^2\delta S}{2} +\pi \pm \f{\pi^2}{4}\f{(g_{\sma{\text{eff}}}-2)^2}{l^2\delta S} + O(B^2). \la{lambdasmallB}}
...
In the strong-hybridization regime: $l^2\delta S {\ll}\pi(g_{\sma{\text{eff}}}{-}2)$, $\lambda_{\pm}{\approx}{\pm}\pi g_{\sma{\text{eff}}} /2$ mod $2\pi$, which corresponds to the Zeeman splitting of Landau levels by $g_{s}\mu_BB$, in the \textit{absence} of the spin-orbit interaction. 
...
\end{equation}
In Fig. \ref{fig:RD}(b), a comparison is made between the Landau levels obtained from exact numerical diagonalization of $H_{RD}(\bK)$ (by standard techniques reviewed in App. \ref{sec:numerical}) and those obtained from the quantization rule. The difference in energies  does not exceed $2\%$ of $\var_c$ [cf.\ \fig{fig:RD}(d)] for various choices of the spin-orbit parameters $(\alpha,\beta)$.
Larger differences are positively correlated with 
$l^2\delta S^2/S$ (with $\delta S$ depending on both $\alpha$ and $\beta$), as shown in Fig. \ref{fig:RD}(c); see also \q{eq:Rashba-exact}, which analytically demonstrates how terms involving $l^2\delta S^2/S$ modifies the quantization rule in the case $\beta=0$. 
...
% beta=0.02
% m=0.01
\caption{Rashba-Dresselhaus model. (a) Typical cyclotron orbits. (b) Comparison of Landau levels calculated by Eq. (\ref{eq:rule}) (black lines) and numerical diagonalization of the Peierls-Onsager Hamiltonian  $H_{RD}(\bK)$ (red dots). Parameters: $\alpha=0.05$(eV$\cdot$\AA), $m=0.01$($\hbar^2$/eV$\cdot$\AA$^2$), $\beta=0.02~(0.04)$(eV$\cdot$\AA) for left (right) panels. $B$ is measured from 2.9 and 1.6T respectively and is in unit $10^{-2}$T. (c-d) For various spin-orbit parameters $(\alpha,\beta)$ of the Rashba-Dresselhaus model and various field strength (satisfying $l^2S(E)\gg 1$), we plot in (c) the hybridization parameter $l^2\delta S(E)$ for fixed energy $(E=0.4$eV${\gg}m\alpha^2)$. For a single Landau level that lies closest to the chosen $E$, we plot in (d) $\text{error}:=|\epsilon_\text{rule}-\epsilon_\text{exact}|/\var_c$, where $\epsilon_\text{rule}$ is determined by Eq. (\ref{eq:rule}) and $\epsilon_{\text{exact}}$ is obtained by numerical diagonalization of the Peierls-Onsager Hamiltonian  $H_{RD}(\bK)$. Parameters: $m=0.01$($\hbar^2$/eV$\cdot$\AA$^2$). Units: $\alpha$($10^{-2}$eV$\cdot$\AA), $\beta$($10^{-2}$eV$\cdot$\AA), $Z$($10^{-3}$eV), $l^2 S$($10^2$).\label{fig:RD}}
\end{figure}
...
let us introduce a Zeeman coupling to an in-plane magnetic field $B_\parallel$:
\e{ 
...
When $|\hbar^2\zpar|>|m\alpha^2|$, the Dirac cone tilts over in the $x$ direction, leading to a Lifshitz transition  of the constant-energy band contours -- from circular to hyperbolic (in the linearized approximation), as illustrated in Fig. \ref{fig:RZ}(c). An over-tilted Dirac cone shall be referred to as type-II. Our case study of a II-Dirac point  connects  two electron-like  pockets [cf.\ \fig{fig:RZ}(a)], and  differs from previous case studies [Obrien, AALG] that connect an electron- and hole-like pocket.

\begin{figure}
\includegraphics[width=1.0\columnwidth]{RZ.png}
\caption{(a) Trefoille-shaped cyclotron orbit for the Rashba-Zeeman model at the energy of the Dirac point. The orbit is drawn over the  {spin-orbit field} ($d_x,d_y$) of the Hamiltonian $H_{RZ}{=}d_0(\bk)I{+}d_x(\bk)\sigma_x{+}d_y(\bk)\sigma_y$, with $d_j$ obtainable from \q{eq:RZ-Hamiltonian}. The wavefunction along the inner (resp.\ outer) component of the trefoille orbit has its pseudospin aligned parallel (resp.\ anti-parallel) to the spin-orbit field. (b) Black dashed lines illustrate a zeroth-order approximation of the Landau levels (assuming Klein tunneling); the leading-order correction is indicated by red lines. The Dirac point lies at $10$meV, at which energy first order correction vanishes in Eq. (\ref{RZfirstorderE}). Parameters: $m$: 0.02$\hbar^2$/eV$\cdot$\AA$^2$, $\alpha$: 0.05eV$\cdot$\AA, $Z_\parallel$: 0.001eV. (c) For various configuration ($\alpha$, $Z_\parallel$) of $H_{RZ}$, we plot in (c) the hybridization parameter $l^2S(E)$ where $E$ is chosen as the energy of the Landau level nearest to the type-II Dirac point, i.e., closest to $\epsilon_0$ in Eq. (\ref{whereisdiracpoint}). We only plotted in region where $0.4<\epsilon_0<1.0$(eV).\red{[Note to Aris: the reason of choosing $\epsilon_0>0.4$ is I want to make sure $l^2S\gg 1$; the reason of choosing $\epsilon_0<1.0$ is Dirac point lying at high energy requires too much harmonics in numerical diagonalization. This requirement is equivalent to $\sqrt{0.8m}\alpha/\hbar<Z<\sqrt{2m}\alpha/\hbar$, corresponding to the boundaries of the plot in (c) and (d).]} (d) $\text{error}:=|\epsilon_\text{rule}-\epsilon_\text{exact}|/\var_c$ corresponding to the parameter configurations in (c), where $\epsilon_\text{rule}$ is determined by Eq. (\ref{eq:rule}) and $\epsilon_{\text{exact}}$ is obtained by numerical diagonalization of the Peierls-Onsager Hamiltonian $H_{RZ}(\bK)$. Parameters: $m$: 0.01$\hbar^2$/eV$\cdot$\AA$^2$, $l=100$\AA. \label{fig:RZ}}
\end{figure}
...
$\calh$ above has been linearized with respect to  $t$, which vanishes in the vicinity of the Dirac point. The eigenbasis of $\sigma_x$ corresponds to two diabatic levels with characteristic slope $v_d{:}{=}\alpha k_E\omega^{\sma{\perp}}_c$; near $t{=}0$, the diabatic levels anticross  due to the hybridization terms ${\propto}\sigma_y,\sigma_z$. Generally, the probability of tunneling across the hybridization-induced barrier is  $\rho^2{=}\exp(-2\pi\barmu)$, with $\barmu{=}{E_g}^2/{2v_d \hbar}$ and $E_g$ the energy gap at the center of the anticrossing;[Landau and lifshitz, Wittig] in this context,
...
$\tilde{\lambda}_{\pm}$ is the slowly-varying semiclassical phase [cf.\ \s{sec:slowvariation}]  acquired by adiabatic transport along the spin-split orbit (denoted $+$ and $-$ for outer and inner orbit respectively). $\tilde{\lambda}_{\pm}$ is generated by  $\H_{\pm \pm}$ and has the same form as \q{phaseindependentorbit}, with $B\int_0^{T_c}\calm_{\sma{\pm\pm}}dt$ equal (in this model) to the single-band Berry phase ($\phi_{\pm}^B$) of orbit $\pm$. At energies far above the Dirac point  ($|E{-}\epsilon_0|{\gg}|Z_{\perp}|$) where the Zeeman interaction is negligible, both orbits encircle the Dirac point and hence $\phi_{\pm}^B{\approx}{\pm}\pi$; far below the Dirac point, neither orbit encircles the Dirac point, leading to $\phi_{\pm}^B{\approx}0$. At intermediate energies, $\phi_{+}^B$ (resp.\ $\phi_{-}^B$) is a continuous and increasing (resp.\ decreasing) function of energy, as supported by the Bloch-sphere argument in Fig. \ref{fig:blochsphere}.
...
\caption{Change of Berry phase across the Zeeman-coupling-gapped Dirac point for inner [(a)] and outer [(b)] orbit. Berry phases can be calculated as half the solid angle enclosed by the corresponding loop on the Bloch sphere. The directions of the loops are indicated by black arrows and the black arrow outside of the sphere indicates increasing energy, where green lines correspond to $\epsilon_0$ in Eq. (\ref{whereisdiracpoint}).\label{fig:blochsphere}}
\end{figure}
...
$\Omega_{\pm}$ is the complete phase acquired by a wavepacket in adiabatically traversing orbit $\pm$, i.e., $\Omega$ sums both rapidly- and slowly-varying components of the semiclassical phase [cf.\ \s{sec:slowvariation}], as well as the quantum Maslov correction ($\gamma_{\pm}{=}\pi$) for circular orbits. A quantization rule analogous to \q{coscos} was previously derived by us for a II-Dirac point that intermediates an electron- and hole-like orbit [AALG]. 
...
For any nonzero field, $0{<}\tau{<}1$ implies that the two incommensurate harmonics $(\Omega_+{\pm}\Omega_-)$ in \q{coscos} compete to produce a quasirandom Landau-level spectrum [kaganovslutskin]. That is to say, the spectrum is disordered on the scale of nearest-neighbor level spacings but has longer-ranged correlations. To appreciate this, let us analyze the Landau levels (near the Dirac point) perturbatively in the small parameter $\tau$. To zeroth order, \q{coscos} reduces to\footnote{ Here, we have neglected $|\phi^B_{+}{+}\phi^B_{-}|$, which is approximately half the relative angle $\theta$ between two incoming trajectories that meet at the II-Dirac point. $\theta/2\pi=?$ \textbf{[please express in terms of Hamiltonian parameters]} and is of order $\delta S/S$. To the accuracy of our quasidegenerate perturbation theory,  $|\phi^B_{+}{+}\phi^B_{-}|$ is therefore negligible compared to the leading term $2l^2S$ in \q{trefoillerule}. } 
\e{2l^2S(E_n^0)+\pi\approx 2\pi n \rightarrow \{E_n^0(B)\}_{n\in\Z}.\label{trefoillerule}} 
 This may be interpreted as a quantization rule for a trefoille-shaped orbit with net action $2S{=}4\pi m E{/\hbar^2}$; the corresponding Landau levels, as plotted with dashed lines in Fig.\ \ref{fig:RZ}(b), are  periodic in $E{\rightarrow}E{+}\pi/l^2(\partial S/\partial E)$ and in $l^2{\rightarrow}l^2{+}\pi/S$. Both periods are approximately half that of either disconnected spin-orbit-split orbit, in the assumed quasidegenerate regime $\delta S{\ll}S$. The $\pi$ correction in \q{trefoillerule} is the Berry phase associated to a $2\pi$ rotation of the wavefunction spin over a single trefoille orbit, as illustrated in Fig. \ref{fig:RZ}(a). The Maslov phase, which is generally equal to $\pi$ times the rotation number of an orbit (i.e., the number of rotations made by a tangent vector over one cycle)[100page], is trivially $2\pi$ for the trefoille orbit.
...
\e{&\delta E_n(B) =  \f{(-1)^{n+1}}{2\pi} \tau \var_c \cos\bigg(\f{l^2\delta S}{{2}}+\f{\phi_+^B-\phi_-^B}{2}-\bar{\varphi}\bigg) \lin
    &\substack{\sma{Z_{\perp}{\rightarrow}0}\\=} \f{(-1)^{n+1}}{4\sqrt{2\pi}}  \var_c \f{(E-\epsilon_0)l}{\alpha\epsilon_0^{3/2}}\cos\bigg(\f{l^2\delta S}{{2}}-\bar{\varphi}\bigg),\label{RZfirstorderE}}
with the right-hand side (of each line) evaluated at $E_n^0(B)$. The second line of \q{RZfirstorderE} is more practically useful where $m{\ll}m_0$. 


%If the out-of-plane Zeeman coupling $\zper$ were zero, $\tau$ vanishes as $|E{-}\epsilon_0|$ in the limit $E{\rightarrow}\epsilon_0$, hence $\delta E_n{\propto}\tau \text{sign}[\epsilon_0-E]$ would be analytic in $B$. However,  nonzero $\zper$ implies that $\tau$ approaches a constant $(\sqrt{2\pi \bar{\mu}_{\sma{\text{min}}}})$ as $E{\rightarrow}\epsilon_0$, leading to a non-analyticity  in $B$ as the Landau level crosses the Dirac point energy. \textbf{[I am not sure that higher-order-in-B corrections may restore analyticity, let's check this numerically]}\red{[I found no evidence of non-analyticity across the Dirac point. Eq. (\ref{RZfirstorderE}) is actually discontinuous across Dirac point, while Eq. (\ref{coscos}) is obviously continuous]}

%$\calh$ above is only defined for $|t|{\ll}T^{\sma{\perp}}_c$. 
%$\calh$ is simplest to interpret in the language of Landau-Zener dynamics:  
%$V_0{:}{=}\sqrt{(Z_\parallel-\alpha k_F)^2{+}Z_{\perp}^2}{>}0$, 


%While tunneling that is localized to type-II Dirac points have been analyzed as a Landau-Zener problem [maxim,beenakker] and is solvable with generalized 
...
In the absence of any symmetry,  the ``non-crossing theorem'' [von Neumann,wigner] states that eigenvalue-degeneracies of complex Hamiltonians are attained by tuning three real parameters; this non-crossing rule applies also to Landau-level degeneracies of the Peierls-Onsager Hamiltonian. \textbf{[Questionable: the original theorem was proven for finite-dimensional Hamiltonians]} Henceforth it should be understood that any parameter we refer to is real, rather than complex. Here, we would prove a stronger statement: in the absence of symmetry, a non-crossing rule also applies to Landau-level quasidegeneracies. Equivalently, we would show that eigenvalue-degeneracies of the unitary propagator [cf. \q{eq:prop}] are attainable by tuning three parameters. Indeed, any two-by-two unitary  can be decomposed as $\A{=}e^{i\phi}{\cal \bar{A}}$, where ${\cal \bar{A}}{\in}\text{SU}(2)$ has eigenvalues $\{e^{i\bar{\lambda}},e^{-i\bar{\lambda}}\}$ which completely determine $\delta \lambda$. Suppose ${\cal \bar{A}}$ is parametrized by $d$ independent real numbers  $\{R_1,\ldots,R_{d}\}{:}{=}\bR $ which are tunable in experiments or on paper. The condition of degeneracy (det$[{\cal \bar{A}}(\bR){\pm}I]{=}0$) defines a set of manifolds in $\bR$-space which are henceforth referred to as degeneracy manifolds. The codimension of the degeneracy manifold  is defined as $d$ minus the dimension of said manifold; intuitively, this codimension is the number of linearly-independent parameters needed to fine-tune a degeneracy. Henceforth, we adopt the shorthand that \textit{codimension} always refers to a codimension of a degeneracy manifold (the exact manifold is to be inferred from the context). In the canonical parametrization of $\text{SU}(2)$ over the three-sphere $S^3$: 
...
Elsewhere, a semiclassical wavepacket undergoes adiabatic (band-conserving) dynamics. For the upper half ($k_y>0$) of the diagram Fig. \ref{fig:Cn-breakdown}(a), this adiabatic evolution is described by a diagonal unitary matrix in the basis of energy bands; each diagonal entry encodes the slowly-varying semiclassical phase due to $\H$ ($\lambda_a'$ \red{[Let's use $\lambda_j'$ instead of $\delta_\lambda$. The latter is confusing because $\lambda_a\sim l^2 \delta S$. It would be quite confusing if we also let $\delta\lambda_a\sim l^2\delta S$]}) acquired along a spin-split trajectory to the \red{top}. In more detail, $\lambda_{\pm}'{=}{\pm}l^2\delta S/4$ up to a field-independent geometric phase \footnote{due to this field-independent phase, it is not necessary that $\delta\lambda_+=-\delta\lambda_-$}; the latter is equal to half the angle swept by the in-plane spin-orbit field $(0,wk_x,\bk^2/2m_2{-}\mu)$ along the spin-split trajectory; cf.\ discussion in \s{sec:Rashba} \red{[Are we really talking about open Berry phase here?]}.  
...
\e{\A&=\red{e^{i\f{\lambda_+'+\lambda_-'}{2}}}\times\nonumber\\
...
\section{Magnetic quantum Oscillations}\label{sec:qo}
