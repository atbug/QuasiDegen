...
\caption{Energy dependence of the single-band Berry phase for inner (upper panel) and outer (lower panel) orbits, denoted $\phi_-^B$ and $\phi_+^B$ respectively. $\phi_{\pm}^B$  equals half the solid angle subtended by the three-vector ${\pm} \bd(\bk)$,[Berry] where  $\bd{=}({-}\alpha k_y$,$Z_\parallel{+}\alpha k_x,{-}Z_\perp$) is obtained from expressing the effective Hamiltonian [cf.\ Eq. (\ref{calHRZ})] as $\sum_jd_j\sigma_j$.  The solid angles are enclosed by blue outlines and the energy is measured from $\epsilon_0$ [cf.\ Eq. (\ref{whereisdiracpoint})].\label{fig:blochsphere}}
\end{figure}
