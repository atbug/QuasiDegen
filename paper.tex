...
with $\cos(\phi)=\text{Re}(\alpha)$. The sufficient and necessary condition for $A_{1/N}\propto I$ is $\text{Re}(\alpha)=\cos(n\pi/N)$, with $n\in\mathbb{Z}$. Tuning $\text{Re}(\alpha)=\cos(n\pi/N)$ requires only one parameter, except for $n=0$ and $n=N$, where three parameters are required.
