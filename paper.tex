\documentclass[aps, prb, showpacs, twocolumn, notitlepage, superscriptaddress, floatfix]{revtex4-1}
\usepackage{amsmath}
...
\caption{\textbf{[please redefine alpha and beta such that it has units of cm per s, in whole paper]} Quantum oscillations in the Rashba-Dresselhaus 2DEG. (a) Evolution of $\delta\lambda$ with respect to the inverse field for different values of $\beta$. The horizontal dashed line indicates $\delta\lambda=\pi$. For $\beta=3.0(10^{5}$cm/s$\cdot\hbar$), we have plotted -- in the upper panel of (b) -- the Landau levels at three $B$ values, as indicated by the vertical dashed lines in (a). The evolution of $\delta \lambda$ for  $\beta=3.0(10^{5}$cm/s$\cdot\hbar$ ) \textbf{[bracket is unconventional notation; try $3.0\times 10^5$ or $3.0\cdot 10^5$]} is given by a cyan-colored line indicated by a black arrow in (a); the three chosen $B$ values correspond approximately to  $\delta\lambda{=}0$, $\delta\lambda{=}\pi/2$ and $\delta\lambda{=}\pi$ respectively. Lower panel of (b) illustrates the saw-tooth oscillation of the chemical potential (at fixed electron density) corresponding to the upper panel. Other parameters chosen in (b): $m{=}0.076m_0$, $\alpha{=}7.5(10^{5}$cm/s$\cdot\hbar$) and $E=0.4$eV.
\label{fig:qo}}
\end{figure}
...
We have extended the semiclassical theory of Landau quantization to describe  quasidegenerate cyclotron orbits, i.e., orbits which are split in $\bk$-space with a differential area ($\delta S$) that is much less than their average area. The quantization rule for two quasidegenerate orbits is summarized in \qq{eq:rule}{berryconn}, and may be applied to spin-degenerate orbits which are split by spin-orbit coupling or by breaking of a crystalline point-group symmetry. The former type of splitting is exemplified by  our case study of the Rashba 2DEG (with arbitrarily-oriented field and addition of Dresselhaus spin-orbit coupling).

The subleading phase corrections ($\lambda_1,\lambda_2$) in our quantization rule  [\q{eq:rule}] encode the dynamical correction due to the splitting of  orbits, as well as a generalized two-band Zeeman interaction. Where $\lambda_1{=}\lambda_2$ mod $2\pi$, there exist a number (of order one) paired Landau levels which are either exactly spin-degenerate, or nearly spin-degenerate in the sense of \qq{llquasideg}{llquasidegB}. \textbf{[bonus:schematic picture to illustrate of order one paired LLs near degeneracy of lambda (might introduce picture earlier, in section where we introduce quasidegeneracy)]} Such paired Landau levels have been referred to as \textit{quasidegenerate}.

For each of the five symmetry classes of near-degenerate orbits \textbf{[please replace quasidegenerate orbit with near-degenerate orbit, however keep quasidegenerate LL. One reason: quasidegenerate LL includes exactly degenerate; near-degenerate orbit has nonzero splitting]},  we have determined the number of tunable real parameters needed to attain a Landau-level quasidegeneracy;  these numbers may be viewed as symmetry-dependent  measures of Landau-level repulsion. We have referred to these numbers as \textit{symmetry-constrained codimensions}. Depending on the symmetry class and representation, the codimension ranges from $0,1,2$ to $3$, as summarized in the right-most column of Tab.\ \ref{table:codimension}.

If the codimension is unity, it is possible to tune the magnitude of the field to attain Landau-level quasidegeneracies; the implications for this tunability -- in the Schubnikov-de Haas effect -- are discussed in \s{sec:quantosc_quasideg}. If the codimension is two, an additional experimentally tunable parameter is needed: e.g., the orientation of the field[magic angle], or the bias voltage in tunneling spectroscopy[cite stm papers].



With the inclusion of nonsymmorphic space-group symmetries, it is not uncommon to find energy bands which are (nearly) four-fold degenerate.[michel and zak,hourglass,Barry] This motivates a  generalization of our rule -- for any number of near-degenerate orbits -- that is presented in  \app{app:quantizationruleproof}. The symmetry-constrained codimensions in the more general case is left to future investigations. 
...
\textbf{[confusing issue to settle: five vs ten becomes confusing if we now say that our topofermiology table applies to all ten classes of quasidegenerate orbits. We may have to modify.][generalization to wilson loop; explains movable crossings in recent works]}
\red{[
Let's say only five rows of both tables are useful for quasidegenerate orbits. This is more accurate since you wrote explicitly the symmetry constraints in terms of sewing matrix in topofermi. These symmetry constraints for the five unticked classes in the topofermi table are not applicable to quasidegenerate orbits. We should just say $\det \A=1$ for the three unticked self-constrained classes for quasidegenerate orbits. This conclusion comes from the fact that we defined $\delta \epsilon$ to be traceless, rather than symmetry constraints expressed with sewing matrix in the topofermi table.
]}
...
Besides the effective Hamiltonian approach described above, semiclassical wave packet theory has been developed in coupled bands[Niucoupled]. It is possible that quantization rule expressed in \qq{eq:rule}{berryconn} can be derived by a `requantization' [NiuRMP] of the wave packet theory.  
