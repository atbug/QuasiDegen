...
%, and the angular magnetoresistance oscillations of semiconductor bilayer heterostructures.      
...
\noi{i} If $|\var_+{-}\var_-|$ is everywhere (along $\frako_0$) much greater than $B|\calm_{+-}|$, then an electron would undergo adiabatic (band-conserving) dynamics \textbf{[Nenciu]} along two independent, nondegenerate orbits. Such an adiabatic regime always exists as $B{\rightarrow} 0$ for nearly-degenerate orbits that do not intersect, e.g, at a type-II Dirac point, which is the subject of the case studies in \s{sec:inplanezeeman} and \s{sec:rotsymmbreakdown}.
...
Our next case study involves orbits that (nearly) touch at a type-II Dirac point\cite{soluyanov_type-ii_2015,muechler_tilted_2016} -- an asymmetric variant of the conventional Dirac point (as will be introduced below). Quantum tunneling localized to a type-II Dirac point  (in short, a II-Dirac point) is known to be mathematically equivalent to  Landau-Zener breakdown, with the electric force replaced by the Lorentz force\cite{AALG,obrien_magnetic_2016}.\textbf{[Blount]} Here, we will demonstrate how Landau-Zener dynamics emerge from our quantization rule [\qq{eq:rule}{eq:H1}], and explore (for the first time) how these dynamics are modified by the Zeeman effect. In particular, we will show that the perfect Klein tunneling predicted by O'Brien \textit{et al}\cite{obrien_magnetic_2016} is never perfect with a proper account of the Zeeman effect. 
...
\caption{\red{A schematic diagram of Landau level dispersion with respect to $B$ for nearly degenerate orbits. One region of Landau level quasidegeneracies (corresponding to $\delta\lambda\equiv 0$) is indicated by a red rectangle.}\label{fig:LL}}
\end{figure}
...
$T_{l^2}$ above is the period (in $l^2$) of quantum oscillations -- in the absence of spin-orbit coupling. To recapitulate, for every exact degeneracy of $\cala$, there must exist, in close proximity, a number of paired Landau levels which are exactly degenerate, or nearly degenerate in the sense of \qq{llquasideg}{llquasidegB}. Each such pair of Landau levels will be called \textit{quasidegenerate}. Landau level quasidegeneracy is sketched in Fig. \ref{fig:LL}.
...
%Generally, such \textit{symmetry-constrained coordinates} are not necessarily related to the canonical coordinates of \q{s3} by a non-singular 

%${\cal \bar{A}}(\br){=}{\pm}I$ defines a set of submanifolds in $\br$-space which we refer to as degeneracy manifolds; in the above example, these manifolds comprise two points on $S^3$.


 
 %\textbf{[there are inconsistencies in these definitions, if we impose $d$ to be number of parameters (without imposing linear independence), then $d=4$ for canonical parametrization, and our definition of codimension as d minus dimension of degeneracy manifold gives 4. however, if we impose linearly-independent parameters, then we also run into trouble if we allow for symmetric coordinates that are not well-defined at points.]}\red{[First of all, I am not quite sure why the word linear independence is used in the current context. I would have used `independent', which means the parameters can be tuned simultaneously. One thing that you would probably dislike is that codimension defined by me would be dependent on the parameters $\br$, so it's OK that codimension in canonical parametrization is three while codimension in exponential parametrization is one. The precise definition of codimension would be the number of independent parameters needed to find a degeneracy. Although codimension depends on the parameters, the codimension found by us is generally universal. For example, for rotation symmetry in class 3, we parametrize $\A_{1/2}$ by canonical parametrization $\br_c$. There exists a mapping from physical parameters $\br$ to $\br_c$. Notice this mapping is not possible for exponential parametrization $\br_e$ since the exponential parametrization of identity is not unique. For a nonpathological parameter choice $\br$, the map is well behaved. The definition of `well-behaved': any $n\le 3$ dimensional manifold in $\br$ is mapped to $n$ dimensional manifold in $\br_c$. With this assumption, codimension of $\A$ in $\br_c$ and $\br$ are actually the same. Exponential parametrization is one example of pathological mapping, which maps to sphere to a point (identity). However, pathological parameter choice usually does not increase codimension. This can be argued in the following way: If the map from $\br$ to $\br_c$ maps a $n\le 3$ dimensional manifold in $\br$ to a $m<n$ dimensional manifold, we remove the corresponding manifold in $\br$. After all such manifolds removed, the map become what I defined as `well-behaved' and the codimension should be the same. If the manifold removed in $\br$ were mapped to $\A\propto I$, then the codimension decreases, which is the case in exponential parametrization. In the presentation, let's be vague and just say codimension is number of independent parameters needed to find a degeneracy.]} alternatively stated, the codimension is the number of linearly-independent parameters needed to fine-tune a degeneracy. Here and henceforth, we adopt the shorthand that  \textit{parameters} will always refer to real parameters, and \textit{codimension} always refers to a codimension of a degeneracy manifold. 
...
%It is well-known that symmetry may reduce the codimension of Hamiltonians, e.g., an antiunitary symmetry may restrict complex Hamiltonians to real Hamiltonians, thus reducing codimension from three to two.[wigner,von neumann] In our context, an analog would be 

%We may ask if these quasidegeneracies (based on band-conservation symmetry) persist at stronger fields where adiabaticity breaks down. If no symmetry remains, then the codimension is three [cf.\ \q{s3}], hence Landau-level quasidegeneracies should not generically be found by varying $B$ alone. 
...
where
\e{\br{=}\bigg(\f{g_sm}{4m_0}{-}\f1{2},0,-\f{\hbar\alpha k_{\sma{E}}}{\var_c}\bigg).}
...
The tracelessness of $\calh$ (at each $\bk$) implies that  ${\A}_{\sma{T_c/2\leftarrow 0}}$  has unit determinant. \red{Since the full propagator $\A$ can be obtained with the knowledge of half of it $\A_{\sma{T_c/2\leftarrow 0}}$, the symmetric coordinates are naturally chosen as the canonical parametrization for this $\text{SU}(2)$ matrix [Eq. (\ref{s3}) with ${\cal \bar{A}}$ replaced by ${\A}_{\sma{T_c/2\leftarrow 0}}$].} In this symmetric coordinates, we derive 
\e{
...
\red{Returning to the Rashba-Dresselhaus model, if the map $f$ from physical variables $(\hbar\alpha k_{\sma{E}}/\var_c,\hbar\beta k_{\sma{E}}/\var_c,g_sm/m_0)$ to the symmetric coordinates is well-behaved, the inverse image of $r_2=0$ in $(\hbar\alpha k_{\sma{E}}/\var_c,\hbar\beta k_{\sma{E}}/\var_c,g_sm/m_0)$ would be a two dimensional manifold, as illustrated in \fig{fig:dgn}(a). On the other hand, ${\cal \bar{A}}{=}{-}I$ occurs if and only if $\br{=}(0,\pm 1,0)$. The inverse image of $f$ of the two points is a path-connected line node confined to the $\alpha{=}0$ and $\beta{=}0$ planes [cf.\ \fig{fig:dgn}(b)];} the difference in codimension originates from the $\mathfrak{c}_{\infty}$ symmetry within these two planes [cf.\ \s{sec:ctsrot}], which was not accounted for in the $\mathfrak{c}_2$-symmetric analysis of \q{proprashba}.
...
\subsubsection{\red{Ten-fold classification of orbits and symmetric coordinates}}\label{sec:reviewtenfold}
...
\red{Among the ten symmetry classes, only six of them (class 1-6) impose self constraints on $\A$. These self constraints can be divided into two types: (i) restricting $\A$ to only a submanifold of SU(2) (class 1, 2, 5, 6); (ii) expressing the propagator $\A$ with only a fraction of it [e.g., Eq. (\ref{eq:sigmazconstraint})] (class 3, 4). For the former case, we define symmetric coordinates by restricting canonical parametrization of $\A$ to that submanifold; for the latter case, we define symmetric coordinates as canonical parametrization of the said fraction of $\A$.}
...
%in that class (and possibly satisfying an additional constraint on the representation of $g$, as specified by the second-to-last column). 
...
Lifshitz-Kosevich formula\cite{lifshitz_kosevich,lifshitz_kosevich_jetp} allow for a harmonic analysis of quantum oscillations, and have previously been derived for  exactly-degenerate orbits in symmetry class 1\cite{rothmag}, with the generalization to all ten symmetry classes in Ref. \onlinecite{topoferm}. A simple generalization of the derivation in \onlinecite{topoferm} extend the utility of these existing formulae to nearly-degenerate orbits. For example, the oscillatory component of the magnetization of 2D metals has the form
\begin{equation}
...
Such beatings have been observed for semiconductor heterostructures \textbf{[das,several more]}; in some experiments, \textbf{[das]} the phase offset $\lambda_a$ [cf.\ \q{eq:LK}] has been measured and fitted phenomenologically to a Laurent series in $B$. \textbf{[das]} It has been appreciated that the leading-order term (${\sim}1/B$) originates from the zero-field splitting of orbits (due to spin-orbit coupling);\textbf{[das]} our contribution is a quantitative theory of the expansion coefficients of this Laurent series -- we have related the subleading term  $({\sim}B^0)$ to a generalized Zeeman interaction, and higher-order terms to a competition between spin-orbit coupling and the Zeeman interaction [cf.\ \q{eq:Rashba-lambda}]. As one application, one may extract the intervals (in $1/B$) between antinodes  for the Rashba 2DEG [by imposing $\delta \lambda{\equiv}\pi$ in \q{eq:Rashba-lambda}] and for the Rashba-Dresselhaus 2DEG (see \s{sec:quantosc_quasideg}).


%Our contribution is the first semiclassical theory  [summarized by the quantization rule of \qq{eq:rule}{berryconn} and the Lifshitz-Kosevich formula of \q{eq:LK}] from which one can calculate the beating pattern; 
...
\red{For two fold exactly-degenerate orbits, three out of five symmetry classes feature $\lambda_1+\lambda_2=0$ with the assumption of SOC can be continuously tuned to zero without changing the degree of degeneracy. Although these three symmetry classes do not impose constraints on nearly-degenerate orbit, due to the traceless spin splitting Hamiltonian matrix, $\lambda_1+\lambda_2=0$ remains true for nearly-degenerate orbits.}

\red{Symmetry constrained codimension discussed in the current work is useful even beyond magnetotransport. Specifically, as a special case of Eq. ({\ref{eq:prop}}) by ignoring Zeeman interaction, Wilson loop share the same symmetry constrained codimension presented in Tab. \ref{table:codimension}. This observation may be helpful in explaining movable crossings in some Wilson loop spectra \textbf{[cite something]}.}

\red{Nontrivial nature of Landau quantization for nearly-degenerate orbits originates from the field induced hybridization of nearly degenerate bands. With the recent development of high magnetic field instrument, we envision quantization rule presented in the current work would be more and more relevant to magnetotransport in future experiments.}
