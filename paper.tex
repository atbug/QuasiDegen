...
\e{&\delta E_n(B) = \f{(-1)^{n+1}}{2\pi} \tau \var_c \cos\bigg(\f{l^2\delta S}{{2}}+\f{\phi_+^B-\phi_-^B}{2}-\bar{\varphi}\bigg) \lin
    &\approx \f{(-1)^{n+1}}{4\sqrt{2\pi}}  \var_c \f{(E-\epsilon_0)l}{\alpha(\epsilon_0/Z_{\parallel})^{3/2}}\cos\bigg(\f{l^2\delta S}{{2}}-\bar{\varphi}\bigg),\label{RZfirstorderE}}
with the right-hand side (of each line) evaluated at $E{=}E_n^0(B)$. The second line is an approximation for $Z_{\perp}{\rightarrow}0$, and has greater utility where $m{\ll}m_0$. \red{Eq. (\ref{RZfirstorderE}) are compared with numerical diagonalization Fig. \ref{fig:RZ}(b) with even integers $n$.}


%( $|\delta \var_+{-}\delta \var_-|{\lesssim}B_\perp|\calm_{\sma{+-}}|$ on average)
...
\e{\A{=}e^{-i2\pi\sum_{j}r_j\tau_j}, \;\;\br{=}\bigg(\f{g_{\sma{\text{eff}}}}{4}{-}\f1{2},0,\f1{2}{-}\f{\alpha k_{\sma{E}}}{\var_c}\bigg).\la{expform}}
Any $SU(2)$ matrix can be expressed in the exponential form [cf.\ \q{expform}]  for some (possibly non-unique) $\br$, with $r_2$ not necessarily zero. We will demonstrate codimension reduction for the general form of $\A$ (having any $\br$), with the Rashba model as a particular realization.


%Nonzero $r_2$ corresponds to a symmetry-allowed deformation that we allow for greater generality -- any $SU(2)$ matrix can be expressed in the exponential form [cf.\ \q{expform}]  for some (possibly non-unique) $\br$, which may be identified as the symmetric  coordinates.  
...
\e{H=\f{{\hbar^2}k^2}{2m_1}+ \bigg(\f{{\hbar^2}k^2}{2m_2}-\mu\bigg)\sigma_z +wk_y\sigma_x,\la{modelC2breakdown}}
with $0{<}m_1{\ll}m_2$ and positive $\mu$. For simplicity, we interpret the Pauli matrices as  spin operators, and the two right-most terms of  \q{modelC2breakdown} as a spin-orbit coupling. The $\mathfrak{c}_2$ symmetry manifests as $\sz H(\bk)\sz{=}H({-}\bk)$, and relates 
two Dirac points at
...
\e{H_{\pm} =\pm {\hbar}\sqrt{\f{2\mu}{m_2}}\bigg(\frac{m_2}{m_1}+\sz\bigg)\delta k_x+w\delta k_{y}\sx+\epsilon_0, \la{linearHam2}}
...
\e{\rho^2=e^{-2\pi\bar{\mu}}, \; \bar{\mu}=\f{\big(Z_{\perp}^2+(m_1/m_2)^2(E-\epsilon_0)^2\big)l^2}{2w\sqrt{2E/m_1}}. \la{landauzenerc2} }
...
\e{\A={e^{i(\tilde{\lambda}_++\tilde{\lambda}_-)}}\left(\matrixtwo{\tau e^{i\bar{\varphi}}}{-\rho}{\rho}{\tau e^{-i\bar{\varphi}}}{\diagmatrix{e^{i\f{\tilde{\lambda}_+-\tilde{\lambda}_-}{2}}}{e^{i\f{\tilde{\lambda}_--\tilde{\lambda}_+}{2}}}} \right)^2. \la{propC2breakdown}}
...
\e{\bk \rightarrow g\circ\bk := (-1)^{s(g)}\check{g}\bk. \la{gmapk}}   
...
%We align the z-axis of our Cartesian coordinate system parallel to the field, such that field-induced dynamics is constrained within planes of constant $k_z$.

%Hence any symmetry relevant in current context must keep $k_x$-$k_y$ plane untouched. 

Given any orbit $\frako_0$ within a plane (given by constant-energy and $k_z$ contours all bands), we may deduce from \q{gmapk} if  $g$ maps the orbit to itself (denoted as $|g\circ\frako_0|=|\frako_0|$) or to a distinct orbit. Moreover, whether $g$ preserves the direction of cyclotron orbit (denoted as $u(g)=0$) or not (denoted as $u(g)=1$) indicates whether the path-ordering in $\A$ needs to be inverted. 
...
\subsubsection{Symmetry-constrained codimension for quasidegenerate orbits}\la{sec:codimquasideg}
...
\subsubsection{Symmetry-constrained codimension for exactly-degenerate orbits}\la{sec:codimexactdeg}
