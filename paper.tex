...
Semiclassical quantization rules are well-known for the Landau levels of  both nondegenerate\cite{Onsager,lifshitz_kosevich,lifshitz_kosevich_jetp} and exactly-degenerate\cite{rotheffham,rothmag,topoferm,100p,Mikitik_quantizationrule} energy bands; the latter is exemplified by the spin-degenerate bands of centrosymmetric solids.
These rules become increasingly accurate as the area of the cyclotron orbit becomes much larger than the inverse square of the magnetic length $l{=}\sqrt{\hbar/eB}$.

Our work generalizes these rules to the Landau quantization of nearly-degenerate bands, whose cyclotron orbits are split in $\bk$-space with a differential area  $\delta S$ that is much less than their average area $S$; $\delta S/S{\ll}1$ shall be referred to as the \textit{near-degeneracy} condition. Nearly-degenerate orbits commonly originate from the spin-orbit-splitting of energy bands, as exemplified by a two dimensional electron gas (2DEG) with Rashba spin-orbit coupling:
\begin{equation}
H_R=\frac{{\hbar^2} k^2}{2m}+\hbar\alpha  (k_{x}\sigma_{y}-k_{y}\sigma_{x}).\label{eq:Rashba-Hamiltonian}
\end{equation}
As illustrated in Fig.\ \ref{fig:orbits}(a), the near-degeneracy condition is satisfied at energies much greater than $m\alpha^2$:\footnote{The expressions for $\delta S$ and $S$ in \q{deltaSvsS} are asymptotically valid for $E{\gg}m\alpha^2$.}
...
By treating both $\delta S/S$ and $1/l^2S$  as small parameters with finite ratio $l^2\delta S$, our perspective diverges from and generalizes previous semiclassical theories\cite{kohn_effham,blount_effham,rotheffham,wannier_fredkin,gao_zero-field_2017,topoferm,100p,Mikitik_quantizationrule} which have only a single, small parameter ($1/l^2S$). $1/l^2\delta S$ measures the hybridization of nearly-degenerate orbits by the Zeeman interaction.
In the regime $l^2\delta S{\gg}1$, the Onsager-Lifshitz quantization rules apply independently to  two concentric (but decoupled) orbits. In the opposite regime $l^2\delta S{\ll}1$, the spin-orbit splitting of energies is negligible, and we may then apply the known quantization rule for exactly-degenerate bands\cite{rotheffham,rothmag,topoferm,100p,Mikitik_quantizationrule}. The goal of this work is to formulate a unifying rule that interpolates smoothly between these two known regimes; such a rule is presented in \s{sec:qtznrules}. 

In \s{sec:Rashba}, we demonstrate the utility of this rule for the Rashba 2DEG with an arbitrarily-oriented magnetic field, and possibly augmented by the Dresselhaus spin-orbit coupling. A sufficiently strong in-plane field not only moves the Dirac point but tilts the Dirac cone over -- into a type-II Dirac point\cite{soluyanov_type-ii_2015}. In \s{sec:inplanezeeman}, we investigate the Landau-Zener tunneling dynamics that occur in the vicinity of a II-Dirac point; in particular, we will critically evaluate a recent claim\cite{obrien_magnetic_2016} of perfect Klein tunneling at the energy of the Dirac point.

A symmetry analysis (cf.\ \s{sec:llquasideg}) of our quantization rule allows us to determine how many tunable parameters are needed to attain a (pseudo-)spin degeneracy between two Landau levels. This degeneracy is exact in symmetry classes where Landau levels are distinguishable by different quantum numbers; absent such quantum numbers, it is still possible for the repulsion between two Landau levels to be anomalously small -- this notion of a Landau-level quasidegeneracy is formulated in \s{sec:relatedegeneracies}. Due to a rotational symmetry of the Rashba-Dresselhaus 2DEG, we find that only a single tunable parameter is needed to attain a Landau-level quasidegeneracy (cf.\ \s{sec:ctsrot}); this parameter can be the magnitude or orientation of the field, or the bias voltage in tunneling spectroscopy. 
...
\caption{A schematic diagram of Landau level dispersion (solid lines) with respect to $B$ for nearly-degenerate orbits. $\delta\lambda$ is plotted in the colored background. Landau level quasidegeneracies occur at $\delta\lambda\equiv 0\equiv 2\pi$.\label{fig:LL}}
\end{figure}
...
A completely different physical realization of two orbits linked by Landau-Zener tunneling was proposed in Ref. \onlinecite{yakovenko_angular_2006} -- for a bilayer semiconductor heterostructure subject to an obliquely-oriented field. The Landau-level quasidegeneracies attained at certain `magic angles' of the field orientation (a single parameter) was theoretically proposed by Yamaji\cite{yamaji_angle_1989}, and experimentally confirmed in Ref. \onlinecite{gusev_interlayer_2008}; however, the role of $\mathfrak{c}_2$ symmetry was not recognized in these works. 
