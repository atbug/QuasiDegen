...
\subsection{Codimension analysis in the absence of symmetry}\la{sec:introducecodimension}
...
${\cal \bar{A}}{=}{\pm}I$ lie on distinct points of $S^3$, and are attainable by tuning three real parameters: $R_3{=}R_4{=}0$, and the phase of $R_1{+}iR_2$ must equal $0$ or $\pi$. The conclusion that three parameters need be tuned remains valid for any  non-singular\footnote{By `non-singular parametrization', we mean a set of parameters related to those in \q{s3} by a non-singular coordinate transformation.} parametrization of SU$(2)$.
...
It is well-known that symmetry may reduce codimension.[wigner,von neumann] The conceptually-simplest example is the approximate symmetry of band conservation, which exists in the adiabatic regime [cf.\ \s{sec:qtznrules}]. Recall from \q{phaseindependentorbit} that  $\cala$ becomes diagonal ($R_3{=}R_4{=}0$) in the basis of energy bands,
with phase[$R_1{\pm}iR_2]{=}{\pm} l^2\delta S/2$ plus field-independent corrections. Since this  phase alone  can be  tuned to attain $\cala{=}{\pm}I$, the codimension is unity.
As a function of $l^2$, we deduce that Landau-level quasi-crossings occur  with a periodicity of $2\pi/\delta S$. This periodicity  has a dual interpretation  in real space -- as a periodic increment of a single flux quantum for  the differential magnetic flux (between the two split orbits).
...
\subsection{Unit codimension for the Rashba 2DEG}\la{sec:singleparameterrashba}

To exemplify the role of rotational symmetry in reducing codimension,
we return to the Rashba model [cf.\ \q{eq:Rashba-Hamiltonian}] absent Dresselhaus spin-orbit coupling and in-plane Zeeman field.  Owing to a continuous rotational symmetry (denoted $\mathfrak{c}_{\infty}$), $\calh(\bk)$ is a constant along the circular zeroth-order orbit [cf.\ \q{rashbaeffham}], and therefore the propagator simplifies to $\A{=}$exp$[{-}i\calh T_c/\hbar]$. The latter is expressible in the exponential form:
\e{\A =e^{-i2\pi(r_1\tau_x{+}r_2\tau_z)}, \;\;(r_1,r_2){=}\bigg(\f{g_{\sma{\text{eff}}}}{4}-\f1{2},\f1{2}-\f{\alpha k_{{E}}}{\var_c}\bigg).\notag} 
All degeneracies occur along circles of radius $\sqrt{r_1^2+r_2^2}{=}{j}/2$, with ${j}$ a nonnegative even (resp.\ positive odd) integer for  $\A{=}I$ (resp.\ $\A{=}{-}I$). In this exactly-soluble model [cf.\ \q{eq:Rashba-exact}], all  degeneracies (${j} {\ge} 0$) of $\A$ correspond to exact Landau-level degeneracies, rather than the near degeneracies described by \qq{llquasideg}{llquasidegB}.

That the ${j}{=}0$ manifold has codimension two can be understood from an elementary result in quantum mechanics: taking $\alpha{=}0$ \textit{and} $m{=}m_0$, we recover the Zeeman-split Landau levels of a free electron gas without spin-orbit coupling; all levels (except the lowest) are spin-degenerate owing to the equality of the Zeeman and cyclotron energies.[LandauLifshitzstatphysI]   All other manifolds ($j{>}0$) have unit codimension, and can be tuned by varying $B$ alone (with fixed $g_{\sma{\text{eff}}}$); these manifolds are analogous to line nodes (of Hamiltonian degeneracies) in 2D $\bk$-space -- a topic of recent interest.[citations] 
...
with $\cos(\varphi)=\text{Re}(\alpha)$. The sufficient and necessary condition for $A_{1/N}^N \propto I$ is $\text{Re}(\alpha)=\cos(n\pi/N)$, with $n\in\mathbb{Z}$. Tuning $\text{Re}(\alpha)=\cos(n\pi/N)$ requires only one parameter, except for $n=0$ and $n=N$, where three parameters are required.
